  ...
  return (
    <li className="chant-item">
      ...
      <div className="options">
        <button className="options-btn" onClick={() => onOptionsClick(chant.id)}>
          ⋮
        </button>
...
const ChantList = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Voulez-vous vraiment supprimer ce chant ?")) {
      ...
      setSelectedChant(null); // Fermer la modale d'informations si elle est ouverte
    }
  };
  ...
  const saveEdit = async () => {
    ...
    const chantDoc = doc(db, "chants", editChant.id);
    await updateDoc(chantDoc, {
      titre: editChant.titre,
      lead: editChant.lead,
      note: editChant.note,
      category: editChant.category,
      youtubeLink: editChant.youtubeLink,
      lyrics: editChant.lyrics,
    });
    setChants((prev) =>
      prev.map((chant) => (chant.id === editChant.id ? editChant : chant))
    );
    setEditChant(null);
      ...
      {selectedChant && (
        <div className="modal">
          <div className="modal-content">
            ...
            <p><strong>📂 Catégorie :</strong> {selectedChant.category?.join(", ")}</p>
            <p>
...
<div className="form-group">
  ...
  <select
    value={editChant.category || "Adoration"} // Valeur par défaut "Adoration"
...
<div className="form-group">
  <label>Note :</label>
  <select
    value={editChant.note || "Do"} // Valeur par défaut "Do"
